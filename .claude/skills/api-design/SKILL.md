...
11. **Document every request body in the generated spec — especially when validation is hand-rolled.** The generated spec is what consumers and client generators actually read; review *it*, not just the handler code. Every write operation (`POST`/`PUT`/`PATCH`) that accepts a body must expose that body's schema **and** an example in the spec. Watch the common trap: when a handler bypasses the framework's typed-body binding — e.g. to map a malformed body to `400` instead of the framework-default `422`, or to parse the raw body itself — the framework can no longer infer the schema, so the spec silently shows an empty or untyped body even though the handler fully validates it. In that case attach the schema+example explicitly (in FastAPI, `openapi_extra=` on the route, built from the same Pydantic model the handler validates against, so the two can't drift). A body that reads as "undocumented" in the spec is indistinguishable from "unvalidated" to a consumer.
...
- **Undocumented request bodies** — a handler that parses/validates its body manually (bypassing the framework's typed-body binding) so the generated spec shows an empty or untyped body, even though every field is validated. Consumers and generated clients then see no schema and assume the endpoint is unvalidated.
