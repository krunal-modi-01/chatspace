...
| Channel + membership service | Create public/private channel (creator = admin); browse public channels (paginated); join/leave; admin-gated membership + role management; last-admin succession; zero-admin frozen state (F29–F37) | New | `backend-engineer` |
...
| WebSocket connection manager | Authenticate token before join; per-conversation subscribe/relay; heartbeat + periodic revalidation; documented close codes; edit/delete/typing/presence event relay across instances (F51–F53, F56, F70) | New | `backend-engineer` |
...
| Invite service | System-Admin issue/revoke/resend single-use 7-day invites; email-locked registration; `410` on non-pending redemption; audit events (F1–F7, ADR-0010) | New | `backend-engineer` |
| Admin / bootstrap service | System-Admin deactivate/reactivate (session + WS invalidation, last-admin guard); env-seeded non-skippable first-run System Admin (F8–F9, F25–F28, ADR-0009) | New | `backend-engineer` |
...
| React SPA | Auth flows, channels/DMs UI, optimistic send, live event rendering + client dedup, reconnect/catch-up banner, media render/download, WCAG 2.1 AA states (PRD §11) | New | `frontend-engineer` |
...
- **WebSocket (WSS)** for real-time only: `message.created` / `message.edited` / `message.deleted`, `typing`, `presence`. Token presented at connect; join only after auth (F52).
