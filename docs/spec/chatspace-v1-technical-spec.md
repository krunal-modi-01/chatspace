...
| React SPA | Auth flows, channels/DMs UI, **My Channels navigation list with live membership updates (F73–F75)**, optimistic send, live event rendering + client dedup, reconnect/catch-up banner, media render/download, **role-gated System Admin screens (Invite Management + User Management, F71/F72, PRD §11)**, WCAG 2.2 AA states (PRD §11). Rendered through the shared design system (`docs/design/DESIGN_SYSTEM.md`) on `architecture/design-tokens.md`, with navigation/IA per `docs/design/INFORMATION_ARCHITECTURE.md` (ADR-0014), a shared full-height conversation surface for channels **and** DMs (ADR-0015/0017), and the workspace user-directory picker for member/DM selection (F76, ADR-0016) | New | `frontend-engineer` |
